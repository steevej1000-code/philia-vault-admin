...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [activeTab, logout, navigate, stats]);
  ...
  const handleBlockUser = async (userId, isFounder = false) => {
    ...
    try {
      ...
      if (isFounder) {
        ...
        setStandardUsers(standardUsers.map(u => u.id === userId ? { ...u, is_blocked: true } : u));
      }
    } catch (err) {
      alert("Erreur lors du blocage : " + (err.response?.data?.error || err.message));
    }
  };
  ...
  const handleUpdateSpots = async () => {
    ...
    try {
      const token = localStorage.getItem('philia_admin_token');
      await axios.put(`${API_BASE}/api/admin/stats/spots`, {
        total: parseInt(newTotal, 10),
        taken: parseInt(newTaken, 10)
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert("Compteur mis à jour avec succès. Veuillez rafraîchir la page.");
      // Optionnel : Forcer le rafraîchissement
  ...
  const handleExport = (type) => {
    const token = localStorage.getItem('philia_admin_token');
    window.open(`${API_BASE}/api/admin/export/${type}?token=${token}`, '_blank');
    // Note: Pour que ça marche avec token en query string, il faudrait adapter le backend 
    // ou utiliser axios pour télécharger le Blob. On va utiliser fetch/axios pour le blob :
    axios.get(`${API_BASE}/api/admin/export/${type}`, {
      headers: { Authorization: `Bearer ${token}` },
      responseType: 'blob'
    }).then(res => {
        ...
        {activeTab === 'overview' && stats && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-philia-card border border-philia-border p-6 rounded-2xl relative">
              <button onClick={handleUpdateSpots} className="absolute top-4 right-4 text-xs font-mono bg-philia-border px-2 py-1 rounded hover:bg-white/10 transition-colors">
                EDIT
              </button>
            ...
            <div className="bg-philia-card border border-philia-border p-6 rounded-2xl">
              ...
              <div className="text-philia-muted font-mono text-[10px] mt-2 tracking-widest">
          ...
          <div className="bg-philia-card border border-philia-border rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-philia-border flex justify-between items-center">
              ...
              <div className="flex space-x-3">
                ...
                <button onClick={() => handleExport('founders')} className="bg-philia-bg text-white font-mono text-xs px-3 py-1 rounded-full border border-philia-border hover:bg-white/10 transition-colors">
                  EXPORT CSV
                </button>
              </div>
            </div>
            {users.length === 0 && !isLoading ? (
              <div className="p-10 text-center text-philia-muted font-mono text-sm">
                No founder members found yet.
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-philia-border">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="p-4">
                          <button onClick={() => handleBlockUser(user.id, true)} className="text-red-400 hover:text-red-300 text-xs font-mono border border-red-900 px-2 py-1 rounded hover:bg-red-900/30 transition-colors">
                            BLOCK
                          </button>
                        </td>
                      </tr>
          ...
          <div className="bg-philia-card border border-philia-border rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-philia-border flex justify-between items-center">
              ...
              <div className="flex space-x-3">
                ...
                <button onClick={() => handleExport('standard')} className="bg-philia-bg text-white font-mono text-xs px-3 py-1 rounded-full border border-philia-border hover:bg-white/10 transition-colors">
                  EXPORT CSV
                </button>
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-philia-border">
                    {standardUsers.map((user) => (
                      <tr key={user.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="p-4">
                          {user.is_blocked ? (
                            <span className="text-red-500 font-mono text-xs">BLOCKED</span>
                          ) : (
                            <button onClick={() => handleBlockUser(user.id, false)} className="text-red-400 hover:text-red-300 text-xs font-mono border border-red-900 px-2 py-1 rounded hover:bg-red-900/30 transition-colors">
                              BLOCK
                            </button>
                          )}
                        </td>
